...
from app.scraping.selectors import DEFAULT_STAY_SITE_CONFIG, StaySiteConfig
from app.scraping.stay_scraper import RawStayResult, StayScraper


class PlaywrightStayAdapter(BaseStayAdapter):
    def __init__(
        self,
        site_config: StaySiteConfig | None = None,
        headless: bool = True,
        max_results: int = 20,
    ) -> None:
        self.site_config = site_config or DEFAULT_STAY_SITE_CONFIG
    ...
    def _to_accommodation_option(
        ...
        return AccommodationOption(
            ...
            source=f"playwright:{self.site_config.name}",
            url=raw.url,
        )
