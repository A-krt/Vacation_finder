...
if __name__ == "__main__":
    ...
    search_url = build_expedia_search_url(destination_query, checkin, checkout, adults)
    screenshot_path = build_screenshot_path(settings.screenshot_dir, prefix="expedia_probe")
    ...
    with BrowserManager(BrowserConfig(headless=True)) as manager:
        ...
        try:
            has_result_card = (
                page.locator("[data-stid='property-listing-results'] > li, section[data-stid='property-listing']").count() > 0
            )
        except Exception:
            has_result_card = False
    ...
    try:
        adapter = ExpediaStayAdapter(headless=True, max_results=5)
    ...
    payload = {
        ...
        "final_url": final_url,
        "title": title,
        "has_result_card": has_result_card,
